...
//print_r($args);exit();
    ...
    private function set_project_tree()
    {
        ...
        if (empty($this->project_tree)) return;
        $available_projects = array_keys($this->project_tree);
        $this->model->set_table('xforms');
        $this->db->where($this->perm_cond);
    ...
    function summary()
    {
        ...
        //xforms
        $this->model->set_table('xforms');
        $xforms = $this->model->get_many_by(['project_id' => $get['project_id']]);
        $data['xforms'] = $xforms;

        $i = 0;
        foreach ($xforms as $value) {
            ...
            $weekly_data[$i] = $this->db->where("submitted_at BETWEEN '$last%' AND '$today%'", NULL, FALSE)->get($value->form_id)->num_rows();

            //daily data
            $daily_data[$i] = $this->db->get_where($value->form_id, array('DATE(submitted_at)' => date('Y-m-d')))->num_rows();
            $i++;
        }

        $data['form_title'] = json_encode($form_title);
        $data['overall_data'] = json_encode($overall_data);
        $data['monthly_data'] = json_encode($monthly_data);
        $data['weekly_data'] = json_encode($weekly_data);
        $data['daily_data'] = json_encode($daily_data);

        //render view
        $this->load->view('project/summary', $data);
    }
