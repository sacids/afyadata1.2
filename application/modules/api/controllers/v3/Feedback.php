...
class Feedback extends REST_Controller
{
    ...
    function index_get()
    {
        ...
        $created_on = $this->get("date_created");
        ...
        if ($user) {
            ...
            $my_perms = $this->user_model->get_my_perms($user->id);
            ...
            //Feedback Mapping
            $this->model->set_table('feedback_mapping');
            ...
            //feedback List
            $feedback_list = $this->feedback_model->find_all(null, $where_array, $created_on);
            ...
            if ($feedback_list) {
                ...
                foreach ($feedback_list as $value) {
                    //user details
                    $this->model->set_table('users');
                    $user = $this->model->get_by('id', $value->created_by);

                    //form details
                    $this->model->set_table('xforms');
                    $form = $this->model->get_by('form_id', $value->table_name);

                    //calculate week number
                    $this->model->set_table($value->table_name);
                    $table = $this->model->as_array()->get_by('meta_instanceID', $value->table_id);

                    //feedback array
                    $feedback[] = array(
                        'id' => $value->id,
                        'form_id' => $value->table_name,
                        'instance_id' => $value->table_id,
                        'title' => $form->title,
                        'message' => $value->message,
                        'sender' => $value->sender,
                        'user' => $user->username,
                        'chr_name' => $user->first_name . ' ' . $user->last_name,
                        'date_created' => date('jS M H:i:s', strtotime($value->created_on)),
                        'status' => $value->status,
                        'reply_by' => $value->replied_by
                    );
                }
                log_message("debug", "messages => " . json_encode($feedback));
                //response
    ...
    function send_post()
    {
        ...
        $user = $this->user_model->get_user_by_username($this->post('username'));
        ...
        if ($user) {
            ...
            //select last feedback message
            $this->model->set_table('feedback');
            $feedback = $this->model->order_by('id', 'DESC')->get_by(['table_id' => $instance_id]);

            log_message("debug", "last feedback message => " . json_encode($feedback));
            ...
            if ($feedback) {
                ...
                $this->model->set_table('feedback');
                $this->model->update_by(['table_id' => $instance_id], ['status' => 'delivered']);

                //save new feedback
                $id = $this->model->insert(
                    [
                        'message' => $this->post('message'),
                        'table_id' => $this->post('instance_id'),
                        'table_name' => $this->post('form_id'),
                        "created_by" => $user_id,
