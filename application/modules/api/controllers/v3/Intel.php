...
class Intel extends REST_Controller
{
    ...
    public function disease_post()
    {
        ...
        if (array_key_exists('specie_id', $req)) {
            ...
            if (array_key_exists($specie_id, $epi_map)) {
                $diseases = array_keys($epi_map[$specie_id]); //todo: check in here
                $diseases = "('" . implode("','", $diseases) . "')";
            } else {
                $this->resp['status'] = '0';
                $this->resp['data'] = 'Specie ID not found';
        ...
        if (array_key_exists('symptoms', $req)) {
            ...
            log_message("debug", json_encode($code));
